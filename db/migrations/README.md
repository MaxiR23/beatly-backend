...
- `025_revoke_anon_execute_scope_helper_policies.sql` — revokes `EXECUTE`
  ...
  `GRANT ... TO anon` the `021` entry left in place (finding 7(a)).
...
- `030_owner_write_policies_for_user_client.sql` — adds the INSERT policy
  ...
  `play_events`/`recent_activity` is drift: `030` does not apply over it,
...
- RLS policies existing in the old database — CLOSED by 017: 32
  ...
  The export gap is closed. Since #143 the user-data domains query as
  the caller, so these policies are enforced; see ADR 010.
